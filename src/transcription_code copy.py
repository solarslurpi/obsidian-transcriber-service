...
class TranscribeAudio:
    ...
    async def transcribe_chapters(self, state: TranscriptionState):
        transcribe_tasks = [asyncio.create_task(self.transcribe_chapter(local_audio_path=state.local_audio_path,
    ...
    async def transcribe_chapter(self, local_audio_path:str, model_name:str, compute_type:torch.dtype, start_time:int, end_time:int):
        try:
            audio_slice = self.make_audio_slice(local_audio_path, start_time*1000, end_time*1000 )
            # # # Load model
        ...
        if audio_slice != local_audio_path:
    ...
    def make_audio_slice(self, local_audio_path:str, start_ms:int, end_ms:int):
