...
async def router(audio_input):
    ...
    if state.local_mp3:
        ...
        asyncio.create.task(TranscribeAudio.transcribe_chapters, state, logger)
        return
    ...
    await asyncio.create_task(TranscribeAudio.transcribe_chapters, state, logger)
